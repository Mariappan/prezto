# Generated by Powerlevel10k configuration wizard on 2019-12-05 at 14:45 +08.
...
() {
  ...
  function prompt_vault() {
    if [[ ${VAULT_ADDR} == *bdrk* ]]; then
        local vault_state=PROD
    elif [[ ${VAULT_ADDR} == *amoy* ]]; then
        local vault_state=STAGING
    else
        local vault_state=DEFAULT
    fi
    p10k segment -f 208 -s $vault_state
  }

  typeset -g POWERLEVEL9K_VAULT_DEFAULT_CONTENT_EXPANSION=''
  typeset -g POWERLEVEL9K_VAULT_STAGING_FOREGROUND=114
  typeset -g POWERLEVEL9K_VAULT_PROD_FOREGROUND=124
  typeset -g POWERLEVEL9K_VAULT_STAGING_VISUAL_IDENTIFIER_EXPANSION='🥶 '
  typeset -g POWERLEVEL9K_VAULT_PROD_VISUAL_IDENTIFIER_EXPANSION='🥵 '
  # typeset -g POWERLEVEL9K_VAULT_PROD_VISUAL_IDENTIFIER_EXPANSION='🔥 🥵' '♼ '
  ...
  function prompt_tmux() {
    # if [[ -v TMUX ]]; then
    if [ -n "${TMUX+1}" ]; then
        local tmux_state=INSIDE
    else
        local tmux_state=OUTSIDE
    fi
    p10k segment -f 208 -s $tmux_state
  }

  typeset -g POWERLEVEL9K_TMUX_INSIDE_FOREGROUND=124
  typeset -g POWERLEVEL9K_TMUX_INSIDE_VISUAL_IDENTIFIER_EXPANSION=''
